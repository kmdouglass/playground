...
def dist_to_time_lut(
    ...
    distances[1:] = np.linalg.norm(np.diff(samples, axis=0), axis=1)  # first element should be 0
...
def plot_curve_with_tangents(ctrl_pts: npt.NDArray[np.float64], curve: npt.NDArray[np.float64]):
...
def uniformly_sample_curve(ctrl_pts: npt.NDArray[np.float64], curve: npt.NDArray[np.float64]):
